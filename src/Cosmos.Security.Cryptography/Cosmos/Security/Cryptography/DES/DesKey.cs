...
{
    public sealed class DesKey : ISymmetricCryptoKeyWithInitializationVector
    {
        ...
        internal byte[] GetIV()
        {
            return SymmetricKeyHelper.ComputeRealValue(IV, null, 64);
        }

        internal byte[] GetIV(byte[] saltBytes)
        {
            var finalSaltBytes = CloneBytes(ref saltBytes);
            return SymmetricKeyHelper.ComputeRealValue(IV, finalSaltBytes, 64);
        }
