...
namespace Cosmos.Security.Verification
{
    internal class SpookyHash2Function : StreamableHashFunctionBase, ISpookyHash
    {
        ...
        public SpookyHash2Function(SpookyHashConfig config)
        {
            if (config == null)
        ...
        public SpookyHashConfig Config => _config.Clone();

        public override int HashSizeInBits => _config.HashSizeInBits;

        public override IBlockTransformer CreateBlockTransformer() => new BlockTransformer(HashSizeInBits, _seed1, _seed2);

        #region Internal Implementation of BlockTransformer

        private class BlockTransformer : BlockTransformerBase<BlockTransformer>
        {
            private static readonly IReadOnlyList<int> _MixRotationParameters = new[] {11, 32, 43, 31, 17, 28, 39, 57, 55, 54, 22, 46};

            private static readonly IReadOnlyList<int> _EndPartialRotationParameters = new[] {44, 15, 34, 21, 38, 33, 10, 13, 38, 53, 42, 54};

            private static readonly IReadOnlyList<int> _ShortMixRotationParameters = new[] {50, 52, 30, 41, 54, 48, 38, 37, 62, 34, 5, 36};

            private static readonly IReadOnlyList<int> _ShortEndRotationParameters = new[] {15, 52, 26, 51, 28, 9, 47, 54, 32, 25, 63};
            ...
            public BlockTransformer() : base(inputBlockSize: 96) { }

            public BlockTransformer(int hashSizeInBits, UInt64 seed1, UInt64 seed2) : this()
            {
