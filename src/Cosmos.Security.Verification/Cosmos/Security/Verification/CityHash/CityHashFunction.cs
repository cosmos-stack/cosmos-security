...
namespace Cosmos.Security.Verification
{
    internal class CityHashFunction : HashFunctionBase, ICityHash
    {
        ...
        public CityHashFunction(CityHashTypes type)
        {
            HashSizeInBits = (int) type;
        }
        ...
        protected override IHashValue ComputeHashInternal(ArraySegment<byte> data, CancellationToken cancellationToken)
        {
            return HashSizeInBits switch
            {
                ...
                128 => ComputeHash128(data, cancellationToken),
        ...
        private IHashValue ComputeHash32(ArraySegment<byte> data, CancellationToken cancellationToken)
        {
            ...
            var hashValue = dataCount switch
            {
                > 24 => Hash32Len25Plus(data, cancellationToken),
                > 12 => Hash32Len13to24(data),
                > 4 => Hash32Len5to12(data),
                _ => Hash32Len0to4(data)
            };
        ...
        private IHashValue ComputeHash64(ArraySegment<byte> data, CancellationToken cancellationToken)
        {
            ...
            var hashValue = dataCount switch
            {
                > 64 => Hash64Len65Plus(data, cancellationToken),
                > 32 => Hash64Len33to64(data),
                > 16 => Hash64Len17to32(data),
                _ => Hash64Len0to16(data)
            };
        ...
        private UInt128 WeakHashLen32WithSeeds(UInt64 w, UInt64 x, UInt64 y, UInt64 z, UInt64 a, UInt64 b)
        {
            a += w;
        ...
        private static UInt64 Mix(UInt64 value) => value ^ (value >> 47);
